...
fn parse_pdbqt_atom(line: &str, line_number: usize) -> Result<Atom, IoError> {
    ...
    let serial = line[6..11].trim().parse::<u32>().map_err(|_| {
        IoError::Parse {
            line: line_number,
    ...
    let residue_num = line[22..26].trim().parse::<u32>().map_err(|_| {
        IoError::Parse {
            line: line_number,
    ...
    // Parse coordinates
    let x = line[30..38].trim().parse::<f64>().map_err(|_| {
        IoError::Parse {
            line: line_number,
    ...
    let y = line[38..46].trim().parse::<f64>().map_err(|_| {
        IoError::Parse {
            line: line_number,
    ...
    let z = line[46..54].trim().parse::<f64>().map_err(|_| {
        IoError::Parse {
            line: line_number,
    ...
    for i in 0..molecule.atoms.len() {
        for j in (i+1)..molecule.atoms.len() {
            ...
            if distance < BOND_THRESHOLD {
                ...
                let rotatable = !matches!(atom1.atom_type, AtomType::Hydrogen | AtomType::HydrogenD) && 
...
pub fn write_docking_results<P: AsRef<Path>>(results: &[crate::optimization::DockingResult], path: P) -> Result<(), IoError> {
    let mut file = File::create(path)?;
    ...
    for (i, result) in results.iter().enumerate() {
        ...
        writeln!(file, "REMARK VINA RESULT:    {:6.3} kcal/mol", result.energy)?;
