...
use rustdock_vina::forcefield::{ForceField, vina::VinaForceField, ad4::AD4ForceField};
use rustdock_vina::io::{parse_pdbqt, write_docking_results};
use rustdock_vina::optimization::{Optimizer, monte_carlo::MonteCarlo};
        ...
        Commands::Dock {
            ...
            let (center, box_size) = match (center, size, config) {
                ...
                _ => {
                    ...
                    return Err(anyhow::anyhow!("Either center and size or a config file must be provided"));
                }
            };
            
            info!("Search box center: ({}, {}, {})", center.x, center.y, center.z);
            info!("Search box size: ({}, {}, {})", box_size.x, box_size.y, box_size.z);
            
            // Parse receptor
            info!("Loading receptor: {}", receptor.display());
            let receptor_molecule = parse_pdbqt(&receptor)
                .with_context(|| format!("Failed to parse receptor file: {}", receptor.display()))?;
            ...
            let flex_molecule = if let Some(flex_path) = flex {
                ...
                Some(parse_pdbqt(&flex_path)
                    .with_context(|| format!("Failed to parse flex file: {}", flex_path.display()))?)
            } else {
                None
            };
            ...
            for ligand_path in ligand {
                ...
                let ligand_molecule = parse_pdbqt(&ligand_path)
                    .with_context(|| format!("Failed to parse ligand file: {}", ligand_path.display()))?;
                ...
                // Run docking
                info!("Docking ligand {} with exhaustiveness {}", ligand_path.display(), exhaustiveness);
                ...
                // Write results
                info!("Writing {} docking poses to {}", filtered_results.len(), output_path.display());
                write_docking_results(&filtered_results, &output_path)
                    .with_context(|| format!("Failed to write docking results to {}", output_path.display()))?;
            }
...
fn parse_config(config_str: &str) -> Result<(Vector3<f64>, Vector3<f64>)> {
    ...
    match (center_x, center_y, center_z, size_x, size_y, size_z) {
        ...
        _ => {
            Err(anyhow::anyhow!("Missing configuration values for center or box size"))
        }
    }
}